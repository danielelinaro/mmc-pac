...
DCBUSINF.Y_PA = -struct2array(load('dcInfAc.mat','Ypa_0'));
DCBUSINF_UNBALANCED.Y_PA = -struct2array(load('dcInfAc_unbalanced.mat','Ypa_0'));

min([angle(DCBUSINF.Y_PA)*CONST angle(DCBUSINF_UNBALANCED.Y_PA)*CONST])
max([angle(DCBUSINF.Y_PA)*CONST angle(DCBUSINF_UNBALANCED.Y_PA)*CONST])
...
for i=1:nx
    for j=1:ny
        ...
        if j==1
            line(DCBUSINF.f, abs(DCBUSINF.Y_PA), 'Color', lincol7, ...
                'LineWidth',  linwid2,...
                'Linestyle','-');
            
            hold on
            
             line(DCBUSINF.f, abs(DCBUSINF_UNBALANCED.Y_PA), 'Color', lincol5, ...
                'LineWidth',  linwid2/2,...
                'Linestyle','-');
        ...
        elseif j == 2
            line(DCBUSINF.f, angle(DCBUSINF.Y_PA)*CONST, 'Color', lincol7, ...
                'LineWidth',  linwid2,...
                'Linestyle','-');
            
            hold on
            
            line(DCBUSINF.f, angle(DCBUSINF_UNBALANCED.Y_PA)*CONST, 'Color', lincol5, ...
                'LineWidth',  linwid2/2,...
                'Linestyle','-');
        ...
        if j==1
            format_ticks(h(i,j),{'$ $','$ $','$ $','$ $','$ $','$ $','$ $','$ $'},...
                ...
                ([0:50:350]), (logspace(-5,0,6)),...
                [],[],[0.020 0.020]);
            axlbl={'$$ $$','$$ |Y_\mathrm{pa}^{(0)}| [\mathrm{S}]$$'};
        ...
        if j==2
            format_ticks(h(i,j),{'$0$','$50$','$100$','$150$','$200$','$250$','$300$','$350$'},...
                ...
                [],[],[0.020 0.020]);
            axlbl={'$$ f [\mathrm{Hz}] $$','$$ \angle Y_\mathrm{pa}^{(0)} [\mathrm{deg}] $$'};
    ...
    if(prn)
        print -loose -painters -depsc2 ..\figs\fig11.eps
    end
end